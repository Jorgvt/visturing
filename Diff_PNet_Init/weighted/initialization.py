...
def init_dn_cs(params, state):
    ...
    params_ = params.copy()
    state_ = state.copy()
    a_star_cs = jnp.load(os.path.join(os.path.dirname(__file__), "a_star_gdn_cs.npy"))
...
def init_dn_v1(params, state):
    ...
    inputs_star = jnp.load(os.path.join(os.path.dirname(__file__), "a_star_gdn_v1.npy"))
